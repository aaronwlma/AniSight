...
def getData():
    ...
    if (str(response) == '<Response [200]>'):
        ...
        with open(user + '.json', 'w') as outfile:
            ...
            print('Data saved as JSON file to working directory as ' + user + '.json')
    else:
        print(user + '.json could not be generated, please try again.')
...
def dumpData():
    ...
    print('\nChecking if ' + user + '.json already exists...')
    mydir = os.getcwd()
    existingJson = False
    ...
    # If the database doesn't exist, create it and call appendNormScore function
        ...
        print(user + '.json does not exist.')
...
def normData():
    ...
    print('\nDumping json data for ' + user  +  'to ' + dbName + '.sqlite...')
    dumpData()
...
def friendComp():
    ...
    conn = sqlite3.connect(dbName + '.sqlite')
    cur = conn.cursor()
    print('\nChecking if ' + user + ' is in ' + dbName + '.sqlite...')
    try:
        cur.execute('SELECT id FROM User WHERE name = ' + user + ';')
    except:
        print(user + ' is not in ' + dbName + '.sqlite.')
        print('\nAdding ' + user + ' to ' + dbName + '.sqlite...')
        normData()
        print(user + ' was added to ' + dbName + '.sqlite.')
